...
def home():
    if 'loggedin' in session:
        cursor = connection.cursor()
        cursor.execute('SELECT * FROM recommendations where user_id = %s limit 50', (session['id'],))
        columns = cursor.description 
...
def uploadFiles():
    ...
    if uploaded_file.filename != '': 
        if uploaded_file.filename[-4:] != ".csv":
            ...
            return render_template('indianajones.html', msg=msg)
        elif data.shape[0] > 200:
            msg = "File exceeds maximum allowed size of 10 KB."
            return render_template('indianajones.html', msg=msg)
        else:
            parseCSV(data)
